...
extension AppDatabase {
    ...
    private func migrate() throws {
        ...
        migrator.registerMigration("1.0.0") { db in
            ...
            try XWHUserDataManager.createUserTable(db)
            try XWHDeviceDataManager.createTables(db)
            try XWHHealthyDataManager.createTables(db)
            try XWHSportDataManager.createTables(db)
        }
