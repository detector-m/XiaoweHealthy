...
class AppUserGuideVC: UIViewController {
    
    typealias AppUserGuideBtnAction = ((_ isSkip: Bool) -> Void)
    ...
    var btnAction: AppUserGuideBtnAction?
...
//        swiftyOnboard?.goToPage(index: 2, animated: true)
        AppUserGuide.isShow = false
        btnAction?(true)
    }
    
    @objc func handleContinue(sender: UIButton) {
        let index = sender.tag
        if index == swiftyOnboardNumberOfPages(swiftyOnboard) - 1 {
            AppUserGuide.isShow = false
            btnAction?(false)
            return
        }
...
extension AppUserGuideVC: SwiftyOnboardDelegate, SwiftyOnboardDataSource {
